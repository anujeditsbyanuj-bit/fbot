...
# via the no-auth web-API/HTML-scrape fallbacks, just without that tier's
# extra reliability. Generate one with Telethon's StringSession (same
# process as ultra-main's own README) and set it as a real SESSION env
# var — NOT hardcoded here like the others above.
#
# BUG FIX: this used to have the same hardcoded-default treatment as
# API_HASH/BOT_TOKEN above, but a Telethon session is a live, currently-
# logged-in identity, not a stable per-project value like a bot token —
# Telegram only allows ONE connection using it at a time. Every
# deployment of this code that didn't set its own SESSION env var
# silently shared this exact same hardcoded one, so running on a second
# server (this VPS) alongside the first (Render) collided with it and
# got telethon.errors.rpcerrorlist.AuthKeyDuplicatedError — Telegram
# forcibly kills a session the moment it sees it active from two places.
# Defaulting to "" instead means an unconfigured deployment now falls
# back to the no-auth resolvers, cleanly and predictably, instead of
