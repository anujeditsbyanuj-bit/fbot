...
def _fetch_terabox_via_token_api(link: str, auth: str) -> dict:
    ...
    status_url = f"{TOKEN_API_STATUS}?link=" + requests.utils.quote(link, safe="")
    poll_interval = 0.5
    for _ in range(90):
...
def _api_call(link: str, dir_path: str = "", page: int = 1, _retry: bool = True) -> list:
    ...
    headers = {"User-Agent": bypass["headers"]["User-Agent"]} if bypass else {}
...
def _first_file(url: str) -> dict:
    """The single-file case get_page_meta/get_available_qualities/
    get_stream_url all need — just the first actual file found at the
    share's root, or one level into its first subfolder if the root is
    all folders (the common "folder containing one video" share shape).
    Raises RuntimeError if nothing resolvable is found.

    Tries the token-API tier first (see the module comment above it) —
    falls through to the web-API + cf_bypass tier on any failure."""
    tok = _resolve_single_file_via_token_api(url)
    if tok:
        return tok
    root_items = _api_call(url, "")
    ...
    for it in root_items:
        if _is_folder_item(it):
            sub_path = it.get("path") or it.get("dirPath") or ""
            for sub_it in _api_call(url, sub_path):
                if not _is_folder_item(sub_it):
                    return sub_it
...
def download_video(url: str, out_path: str,
    ...
    try:
        ...
        it = _first_file(url)
        fresh_link = it.get("downloadLink") or it.get("streamUrl")
