...
# BUG FIX: _api_call() used to send its post-cf_bypass-solve retry
# through this SAME _session object too, just without the `impersonate`
# kwarg on that one call — on the theory that omitting it per-request
# was enough to stop presenting a Chrome-impersonated TLS fingerprint
# that doesn't match whatever fingerprint cloudscraper actually solved
# the challenge with. It wasn't: curl_cffi's Session keeps its own
# connection pool, and the FIRST request on this session (the one that
# actually gets the initial 403, made WITH impersonate="chrome") already
# opens a chrome-fingerprinted TLS connection that the Session then
# reuses for later requests on it regardless of that later request's own
# kwargs — confirmed live: cf_bypass logged "✅ solved... cached", and
# the very next call still 403'd. A plain requests.Session, kept
# entirely separate from the curl_cffi one, guarantees the post-solve
# retry can't inherit a pooled Chrome-impersonated connection from the
# attempt before it.
_plain_session = requests.Session()
_plain_session.headers.update(dict(_session.headers))
...
# ─────────────────────────────────────────────────────────────────────────
try:
    from diskwala import get_auth_token_sync, DiskwalaAuthError, decrypt_file, _pick, _invalidate_auth_token as _invalidate_diskwala_token
...
def _fetch_terabox_via_token_api(link: str, auth: str) -> dict:
    ...
    for _ in range(90):
        ...
        if status == "done":
            ...
            # BUG FIX: this used to only check 2-3 field-name variants per
            # value ("name"/"fileName", "downloadUrl"/"url", etc.) — far
            # fewer than diskwala.py's own _fetch_diskwala_video_via_api
            # checks for the exact same api2.diskwala.net response shape
            # (this function's own docstring claims to match it, but the
            # response-parsing side had drifted out of sync). Now shares
            # that same _pick() call, so a response using "title" instead
            # of "name", "download_url" (snake_case) instead of
            # "downloadUrl", "thumbnailUrl"/"poster"/"image" instead of
            # "thumb", etc. — all things diskwala.py's version already
            # handles — get caught here too instead of silently
            # returning None for that field.
            return {
                "fileName": _pick(file, "name", "fileName", "filename", "title") or "file",
                "fileSize": _pick(file, "size", "fileSize", "length") or 0,
                "downloadLink": _pick(file, "downloadUrl", "download_url", "url", "link"),
                "streamUrl": _pick(file, "streamUrl", "stream_url", "hls")
                             or _pick(file, "downloadUrl", "download_url", "url", "link"),
                "thumbnail": _pick(file, "thumb", "thumbnail", "thumbnailUrl", "poster", "image"),
                "duration": _pick(file, "duration", "duration_seconds", "durationSeconds"),
                "isDir": False,
                "type": "video",
            }
...
# ─── terabox.beer resolver ───────────────────────────────────────────────────
_BEER_BASE      = "https://terabox.beer"
_BEER_WATCH_URL = "https://terabox.beer/watch/{vid}"
_BEER_API_URL   = "https://terabox.beer/api/terabox-new"
_BEER_UA        = (
    "Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 "
...
_beer_session: requests.Session | None = None


def _get_beer_session() -> requests.Session:
    """Singleton session for terabox.beer with warmed-up cookies."""
    global _beer_session
    if _beer_session is None:
        s = requests.Session()
        s.headers.update(_BEER_HEADERS)
        try:
            # Step 1: warm up homepage session
            s.get(_BEER_BASE + "/", timeout=12, verify=False)
        except Exception as e:
            logger.debug(f"[beer] homepage warmup failed: {e}")
        _beer_session = s
    return _beer_session


def _resolve_via_beer(terabox_url: str) -> dict | None:
    """Resolve a Terabox URL through terabox.beer's free API.

    Request flow (from the API spec):
      1. Extract video ID from /s/<id> pattern
      2. GET terabox.beer/watch/<id>  — warm up watch-page session
      3. GET terabox.beer/api/terabox-new?link=<encoded_url>
      4. Parse JSON → extract download URL

    Returns {"proxy_url", "file_name", "file_size"} or None on failure.
    """
    # Extract video ID
    m = _BAIDU_SURL_RE.search(terabox_url)
    if not m:
        return None
    vid = m.group(1)
    ...
    s = _get_beer_session()
    try:
        # Step 2: warm up watch page
        watch_url = _BEER_WATCH_URL.format(vid=vid)
        s.get(watch_url, timeout=12, verify=False,
              headers={"Referer": _BEER_BASE + "/"})
    except Exception as e:
        logger.debug(f"[beer] watch-page warmup failed: {e}")

    try:
        # Step 3: call the resolver API
        api_url = _BEER_API_URL
        resp = s.get(api_url,
                     params={"link": terabox_url},
                     timeout=20,
                     verify=False,
                     headers={"Referer": watch_url, "Accept": "application/json"})
        resp.raise_for_status()
    except Exception as e:
        logger.warning(f"[beer] API request failed: {e}")
        return None

    try:
        data = resp.json()
    except Exception:
        logger.warning(f"[beer] non-JSON response: {resp.text[:200]}")
        return None

    # Error check
    if not isinstance(data, dict) or data.get("error"):
        logger.debug(f"[beer] API returned error: {str(data)[:200]}")
        return None

    # Step 6: extract URL in priority order
    url_fields = [
    ...
    # Step 7: fallback — scan any string value
    if not proxy_url:
    ...
    if not proxy_url:
        logger.debug(f"[beer] no usable URL in response: {str(data)[:200]}")
        return None

    # Step 8: follow redirects and detect .m3u8
    try:
        head = s.head(proxy_url, allow_redirects=True, timeout=12, verify=False)
        proxy_url = head.url  # final URL after redirects
    except Exception:
        pass

    result = {
        "proxy_url":  proxy_url,
        "file_name":  data.get("file_name") or data.get("fileName") or "video.mp4",
        "file_size":  data.get("file_size") or data.get("fileSize") or 0,
    }
    logger.info(f"[beer] resolved {terabox_url[:60]} → {proxy_url[:80]}")
    return result
...
def _api_call(link: str, dir_path: str = "", page: int = 1, _retry: bool = True) -> list:
    ...
    try:
        ...
        # BUG FIX: see _plain_session's own comment above this function —
        # once cf_bypass has solved the challenge and handed us real
        # cf_clearance cookies, this goes out over a completely separate
        # plain requests.Session (never impersonated, never shared with
        # the curl_cffi one below) so it can't inherit a pooled Chrome-
        # impersonated TLS connection from an earlier request on this
        # process. Only the FIRST, pre-solve attempt (no cookies yet)
        # uses the curl_cffi session with impersonation.
        if cookies:
            resp = _plain_session.post(_API_URL, **post_kwargs)
        else:
            if _CURL_CFFI_AVAILABLE:
...
def _first_file(url: str, _dir_path: str = "", _depth: int = 0) -> dict:
    ...
    Tier order: terabox.beer (free, fastest) → token-API (diskwala.net)
    → web-API (diskwala.net Baidu PCS pipeline).
    ...
    tier on any failure."""
    if _depth == 0:
        # Tier 1: terabox.beer — free resolver, no session/auth needed
        beer = _resolve_via_beer(url)
        if beer and beer.get("proxy_url"):
            return {
                "downloadLink": beer["proxy_url"],
                "streamUrl":    beer["proxy_url"],
                "fileName":     beer.get("file_name") or "video.mp4",
                "fileSize":     beer.get("file_size") or 0,
            }

        # Tier 2: diskwala.net token-API
        tok = _resolve_single_file_via_token_api(url)
        if tok:
            return tok
