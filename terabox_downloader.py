...
def _api_call(link: str, dir_path: str = "", page: int = 1, _retry: bool = True) -> list:
    ...
    try:
        ...
        # BUG FIX: this used to add impersonate="chrome" (curl_cffi's own
        # TLS stack, a from-scratch BoringSSL implementation of Chrome's
        # exact fingerprint) unconditionally — including on the retry
        # right after cf_bypass solved a challenge. But that solve
        # happened via cloudscraper, a completely different library built
        # on `requests`'s own TLS stack with its own cipher-suite
        # ordering — a different fingerprint that just happens to also
        # call itself "chrome-like". Cloudflare's cf_clearance cookie is
        # tied to the fingerprint that solved the challenge; presenting
        # the same cookie over a DIFFERENT fingerprint reads as a replayed/
        # stolen cookie and gets an instant fresh 403 — confirmed from a
        # real log showing "✅ solved... cached" immediately followed
        # (24ms later) by the exact same request 403ing again. Skip
        # curl_cffi impersonation specifically when we're presenting
        # cf_bypass cookies, so the TLS handshake stays consistent with
        # whichever client (cloudscraper) actually solved the challenge.
        if _CURL_CFFI_AVAILABLE and not cookies:
