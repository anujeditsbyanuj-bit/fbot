...
# FlareSolverr (see cf_bypass.py + the Dockerfile's own comment above its
# git-clone step) — a real headless-Chrome Cloudflare-challenge solver
# every scraper backend falls back to via `import cf_bypass`. Needs a
# virtual display (it drives an actual, non-headless-flagged Chrome for
# better stealth against Cloudflare's own headless detection) — Xvfb was
# already apt-installed alongside chromium in the Dockerfile for exactly
# this. Same start-in-background-then-poll pattern as bgutil-pot/ytnode
# above; a failure here is non-fatal the same way — cf_bypass.py's own
# try_solve_flaresolverr() already handles "can't reach FlareSolverr" by
# just reporting no bypass available, same as every caller already
# handles cf_bypass being missing entirely.
Xvfb :99 -screen 0 1024x768x24 -nolisten tcp > /tmp/xvfb.log 2>&1 &
export DISPLAY=:99

HOST=127.0.0.1 PORT=8191 LOG_LEVEL=info \
    /opt/flaresolver/venv/bin/python /opt/flaresolver/src/flaresolverr.py > /tmp/flaresolverr.log 2>&1 &
FLARESOLVERR_PID=$!
...
if [ "$FLARESOLVERR_UP" = "1" ] && kill -0 "$FLARESOLVERR_PID" 2>/dev/null; then
    echo "[flaresolverr] OK — Cloudflare-challenge solver is up on :8191"
else
    echo "[flaresolverr] WARNING — did not come up after 15s. Last log lines:"
