...
if not _CF_BYPASS_AVAILABLE:
    logger.warning(
        "cf_bypass module not found (deleted?) — Cloudflare-JS-challenge "
        "403s won't get an automatic FlareSolverr retry; everything else "
...
def _cookies_for(url: str):
    ...
    if ("you-porn.com" in host or "youporn.com" in host) and config.YOUPORN_COOKIES and os.path.exists(config.YOUPORN_COOKIES):
        return config.YOUPORN_COOKIES
    if "spankbang.com" in host and config.SPANKBANG_COOKIES and os.path.exists(config.SPANKBANG_COOKIES):
        return config.SPANKBANG_COOKIES
    if "beeg.com" in host and config.BEEG_COOKIES and os.path.exists(config.BEEG_COOKIES):
        return config.BEEG_COOKIES
...
def _base_opts(url: str = "") -> dict:
    ...
    elif url and re.search(r"you-?porn|youporn", url, re.I):
        # YouPorn redirects to an age-gate page without session cookies —
        # yt-dlp downloads the tiny gate page (~600B) instead of the video.
        # age_limit=99 tells yt-dlp to skip age-gate checks and attempt
        # the video URL directly; combined with a valid cookiefile (set
        # via YOUPORN_COOKIES env var) this bypasses the gate entirely.
        opts["age_limit"] = 99
        opts["http_headers"] = {"Referer": "https://www.youporn.com/"}
        opts.setdefault("extractor_args", {})["YouPorn"] = {"age_gate": ["bypass"]}
    elif url and re.search(r"spankbang", url, re.I):
        # SpankBang needs a Referer for CDN hotlink protection.
        opts["age_limit"] = 99
        opts["http_headers"] = {"Referer": "https://spankbang.com/"}
    ...
    if url and _CF_BYPASS_AVAILABLE:
...
def _extract_info(video_url: str) -> dict:
    ...
    try:
        ...
        # FIX 1: yt-dlp's own YoutubeDL.extract_info() catches an
        # extractor's raw ExtractorError internally and re-raises it as
        # DownloadError (report_error() -> trouble() -> "raise
        # DownloadError(...)") — so DownloadError, not ExtractorError, is
        # what actually reaches caller code in practice. The previous
        # `except yt_dlp.utils.ExtractorError` here never matched at all
        # for that common case, so this whole CF-bypass block was
        # unreachable dead code for it — confirmed from a real SpankBang
        # 403 traceback that propagated all the way up unhandled instead
        # of ever reaching this except clause.
        err = str(e)
        # FIX 2: the "cloudflare"/"challenge" wording check below assumed
        # yt-dlp's error text always names Cloudflare explicitly — it
        # doesn't. SpankBang's extractor (and plenty of others) just
        # surfaces a generic "HTTP Error 403: Forbidden", no CF-specific
        # wording at all, even when the actual cause IS a Cloudflare JS
        # challenge (cf_bypass.py's own docstring confirms this exact
        # site needed FlareSolverr). Narrowed matching therefore silently
        # skipped exactly the case it exists to catch. Trying on any 403
        # is safe regardless: cf_bypass.try_solve() already fails
        # gracefully and quickly (a single POST) if FlareSolverr isn't
        # reachable, or can't solve, or the block isn't actually a CF
        # challenge — same "just surface the original error" fallback as
        # before, just reached less selectively now.
        looks_like_cf_challenge = "403" in err
        if looks_like_cf_challenge and _CF_BYPASS_AVAILABLE and cf_bypass.try_solve(video_url):
            opts = {**_base_opts(video_url), "skip_download": True}
            opts["socket_timeout"] = 10
            opts["retries"] = 2
            opts["fragment_retries"] = 1
