...
def get_stream_public_url(code: str) -> str:
    ...
    """
    # ── Explicit public URL (recommended for VPS/reverse proxy) ─────
    # Example: PUBLIC_BASE_URL=https://example.com
    # or:     PUBLIC_BASE_URL=http://77.90.19.196:8080
    explicit = os.environ.get("PUBLIC_BASE_URL", "").strip().rstrip("/")
    if explicit:
        return f"{explicit}/stream/{code}"
...
class _StreamingHandler(BaseHTTPRequestHandler):
    ...
    def _handle_stream(self, is_head: bool):
        ...
        if len(parts) < 2 or parts[0] != "stream":
            ...
            self.end_headers()
            if not is_head:
                self.wfile.write(f"Anujkumar alive | port={_PORT}".encode())
            return
...
def Anujkumar_keep_alive(real_server_started: bool = False):
    ...
    with _LOCK:
        if not real_server_started and _SERVER is None:
            try:
                # Bind on all interfaces so http://PUBLIC_IP:PORT/stream/... is reachable.
                # ThreadingHTTPServer lets multiple Telegram/browser clients stream
