...
def get_session():
    """A fresh session for a caller that needs to make MULTIPLE requests
    as one stateful flow — e.g. a GET to pick up a CSRF token + session
    cookie from a page, then a POST that must present both back (Laravel
    apps like flowvideoplayer.com's /telegram/bot/search/video work this
    way). request() above is for one-off calls and doesn't persist
    cookies between separate calls itself, which breaks exactly this kind
    of flow.

    Returns a curl_cffi Session impersonating Chrome's TLS fingerprint
    when available (see this module's docstring for why that matters
    against Cloudflare), otherwise a plain requests.Session — still fine
    for a site that doesn't fingerprint TLS at all; only Cloudflare-grade
    protection actually needs the impersonation. Never raises: a curl_cffi
    session that fails to construct (see _curl_cffi_session's own
    fallback list) just means a plain requests.Session instead."""
    if _CURL_CFFI_AVAILABLE:
        try:
            return _curl_cffi_session()
        except Exception as e:
            logger.info(f"cf_bypass.get_session: curl_cffi session failed ({e}), falling back to plain requests.Session")
