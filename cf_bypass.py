...
def _try_solve_cloudscraper(url: str) -> bool:
    ...
    try:
        ...
        logger.debug("[cf-bypass] cloudscraper not installed — skipping straight to FlareSolverr.")
        return False

    domain = _domain_of(url)
    try:
        scraper = cloudscraper.create_scraper()
        resp = scraper.get(url, timeout=_SOLVE_TIMEOUT)
        resp.raise_for_status()
    except Exception as e:
        logger.debug(f"[cf-bypass] cloudscraper couldn't solve {domain}: {e}")
        return False
    ...
    if not any(c["name"] == "cf_clearance" for c in cookies):
        logger.debug(f"[cf-bypass] cloudscraper got a response for {domain} but no cf_clearance cookie — likely not a Cloudflare challenge, or unsolved.")
        return False
