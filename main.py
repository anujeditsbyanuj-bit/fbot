...
async def debughtml_cmd(client: Client, m: Message):
    ...
    elif diskwala.is_playlist_link(url):
        html = await asyncio.to_thread(diskwala.fetch_debug_html, url)
    else:
        await status.edit_text(SC("⚠️ <b>Not a recognized faphouse/fpo.xxx/diskwala playlist link.</b>"), parse_mode=ParseMode.HTML)
        return
...
@app.on_message(filters.command("flarestatus") & filters.private)
async def flarestatus_cmd(client: Client, m: Message):
    """BUG FIX: flaresolver_bootstrap.py had is_ready() but no
    get_status() — no way to tell, from the bot itself, WHY a
    Cloudflare-gated site (e.g. diskwala.net's terabox API — see a real
    log: "non-JSON response (status 403) — likely a Cloudflare challenge
    cf_bypass couldn't solve") kept failing: missing /opt/flaresolverr
    build, Xvfb not starting, Chromium itself not launching, or FlareSolverr
    just never coming up within its startup window — all four look
    identical from the outside (a plain 403 downstream) without this.
    Same shape as /potstatus above."""
    if m.from_user.id not in ADMINS:
        return await m.reply(SC("🚫 Owner/admin only."), parse_mode=ParseMode.HTML)
    ready = flaresolver_bootstrap.is_ready()
    status = flaresolver_bootstrap.get_status()
    icon = "🟢" if ready else "🔴"
    await m.reply(SC(
        f"{icon} <b>FlareSolverr:</b> {'ready' if ready else 'NOT ready'}\n"
        f"📋 <b>Status:</b> <code>{status}</code>\n\n"
        + ("cf_bypass.py's second tier (after cloudscraper) — a real headless "
           "Chromium that solves Cloudflare's JS challenge for sites cloudscraper's "
           "lighter JS-VM can't handle, e.g. diskwala.net's terabox API. "
           if ready else
           "Without this, any site whose Cloudflare challenge cloudscraper alone "
           "can't solve (see cf_bypass.py's own \"cloudscraper got a response but no "
           "cf_clearance cookie\" warning in the logs right before a request like this "
           "fails) has no second tier to fall back to — it just 403s. Check the raw "
           "logs for lines starting with <code>[flaresolverr]</code> around this "
           "status message for exactly which setup step failed.")
    ), parse_mode=ParseMode.HTML)
...
LINK_HANDLER_FILTER = (
    ...
    & ~filters.command(["start", "help", "myplan", "plans", "premium", "addpremium",
                                          ...
                                          "addbot", "delbot", "titanium", "potstatus"])
        ...
        try:
            ...
            async with download_semaphore(priority=premium["is_premium"]):
                final_path = await asyncio.to_thread(
                    ...
                    terabox.download_video, link, out_path, None, f["url"], True,
                )
...
async def process_link(client: Client, m: Message, link: str, tag: str):
    ...
    if diskwala.is_playlist_link(link):
        status = await m.reply(
            SC("📋 <b>Fetching Diskwala playlist...</b>"),
            parse_mode=ParseMode.HTML,
        )
        try:
            info = await asyncio.to_thread(diskwala.fetch_playlist_info_with_auth_retry, link)
        except diskwala.DiskwalaAuthError as e:
            await status.edit_text(
                SC(f"<b>⚠️ Auth failed:</b> <code>{e}</code>\n\nCheck SESSION env var."),
                parse_mode=ParseMode.HTML,
            )
            return
        except Exception as e:
            await status.edit_text(
                SC(f"<b>❌ Could not fetch playlist:</b>\n<code>{str(e)[:400]}</code>"),
                parse_mode=ParseMode.HTML,
            )
            return
        ...
        if not files:
            await status.edit_text(
                SC(f"<b>📋 {html.escape(title)}</b>\n\n❌ No files found in this playlist."),
                parse_mode=ParseMode.HTML,
            )
            return
        ...
        await status.edit_text(
            SC(
                ...
                f"⬇️ Downloading all files to channel..."
            ),
            parse_mode=ParseMode.HTML,
        )
        ...
        for idx, file_entry in enumerate(files, 1):
            file_link = file_entry.get("link") or file_entry.get("url")
            file_name = file_entry.get("name") or f"file_{idx}"
            if not file_link:
                failed_n += 1
                continue
            try:
                await status.edit_text(
                    SC(
                        f"<b>📋 {html.escape(title)}</b>\n"
                        f"⬇️ [{idx}/{len(files)}] <code>{html.escape(file_name)}</code>"
                    ),
                    parse_mode=ParseMode.HTML,
                )
                video_info = await asyncio.to_thread(
                    diskwala.resolve_diskwala_with_retry, file_link
                )
                download_url = (
                    video_info.get("downloadUrl")
                    or video_info.get("download_url")
                )
                if not download_url:
                    raise RuntimeError("No download URL in response")

                safe_name = re.sub(r'[\\/:*?"<>|\r\n]+', " ", file_name).strip()[:120] or f"file_{idx}"
                if not any(safe_name.lower().endswith(ext) for ext in (".mp4", ".mkv", ".webm", ".mov", ".avi", ".m4v")):
                    safe_name += ".mp4"

                out_path = os.path.join(DOWNLOAD_DIR, f"dw_pl_{uuid.uuid4().hex[:8]}_{safe_name}")
                await asyncio.to_thread(
                    diskwala.download_video, file_link, out_path,
                    on_progress=None, stream_url=download_url
                )
                ...
                await client.send_video(
                    m.chat.id, out_path,
                    caption=SC(f"<b>{html.escape(safe_name)}</b>\n📃 {idx}/{len(files)}"),
                    parse_mode=ParseMode.HTML,
                    supports_streaming=True,
                )
                done_n += 1
            except Exception as e:
                logger.warning(f"[diskwala-playlist] {file_name} failed: {e}")
                failed_n += 1
            finally:
                try:
        ...
        summary = (
            f"✅ <b>Playlist done — {done_n}/{len(files)} uploaded.</b>"
            + (f"\n⚠️ {failed_n} file(s) failed and were skipped." if failed_n else "")
        )
...
async def show_quality_menu(client: Client, query, link_id: str, link: str, page: int = 0):
    ...
    if poster_url:
        ...
        #
        # FIX: passing poster_url straight through as `photo=` means
        # TELEGRAM's own servers fetch it, not us — so we can't attach a
        # custom Referer to that request. TeraBox's thumbnail CDN
        # (data.teraboxpage.com) specifically requires one (see
        # download_thumb()'s docstring) and silently fails without it,
        # meaning every TeraBox quality-menu thumbnail was quietly
        # dropping to the text-only fallback below. For TeraBox links,
        # download it ourselves first (with the right Referer) and
        # upload that local file instead — every other backend keeps the
        # cheaper direct-URL path unchanged, since only this one CDN
        # actually needs this.
        photo_arg = poster_url
        temp_thumb_path = None
        if _downloader_for(link) is terabox:
            temp_thumb_path = os.path.join(DOWNLOAD_DIR, f"qmenu_thumb_{link_id}.jpg")
            got = await asyncio.to_thread(download_thumb, poster_url, temp_thumb_path, TERABOX_THUMBNAIL_REFERER)
            photo_arg = temp_thumb_path if got else None

        if photo_arg:
            try:
                await client.send_photo(
                    query.message.chat.id, photo=photo_arg,
...
def download_thumb(thumb_url: str, thumb_path: str, referer: str = None) -> bool:
    """Download a thumbnail image from a URL.

    FIX: sent no headers at all — fine for most sites, but TeraBox's
    thumbnail CDN (data.teraboxpage.com, the "thumbnail" field
    diskwala.net's API returns) specifically checks Referer and won't
    serve the real image without it (confirmed via a real captured
    request: Referer: https://diskwala.net/, unrelated to which TeraBox
    mirror domain the original share link used — this is the CDN's own
    fixed requirement, not per-link). referer is optional so every other
    caller/site is unaffected."""
...
# TeraBox thumbnails (data.teraboxpage.com, via diskwala.net's API) need
# this exact Referer to be served — see download_thumb()'s docstring.
TERABOX_THUMBNAIL_REFERER = "https://diskwala.net/"
...
async def _download_faphouse_video_inner(client: Client, query, link: str, status_msg, chat_id: int,
    ...
    try:
        ...
        if not got_thumb:
            ...
            poster_url = page_meta.get("poster_url")
            if poster_url:
                thumb_referer = TERABOX_THUMBNAIL_REFERER if backend is terabox else None
                got_thumb = await asyncio.to_thread(download_thumb, poster_url, thumb_path, thumb_referer)
        if not got_thumb:
...
async def send_stream_link(client: Client, query, link: str):
    ...
    if diskwala.is_diskwala_link(link):
        try:
            ...
            # BUG FIX: this used to call register_stream_proxy() with no
            # referer= at all, so every Diskwala/Flezen/VidBunker stream
            # link got proxied with the hardcoded flezen.com default
            # regardless of which site it actually came from — VidBunker
            # and Diskwala's own CDNs check for their own origin and 403
            # a mismatched one. get_referer_for_link() already knows the
            # right origin per site (including VidBunker's special case:
            # vidbunker.net, not the .in share-link domain).
            register_stream_proxy(code, cdn_url, name, size, referer=diskwala.get_referer_for_link(link))
