...
async def _send_diskwala_playlist(client: Client, chat_id: int, user_id: int, playlist_url: str):
    ...
    try:
        ...
        # FIX: this used to show only a generic "couldn't fetch" message,
        # with the real reason going to the server logs alone (the
        # logger.warning just above) — every other resolver failure in
        # this bot (single Diskwala/Flezen files, Faphouse, etc.) shows
        # str(e) directly in the reply so the person doesn't have to ask
        # for logs to find out why. Same here now.
        await client.send_message(
            chat_id,
            SC(
                "📋 <b>Couldn't fetch this playlist right now.</b>\n"
                f"<code>{_strip_ansi(str(e))[:500]}</code>\n\n"
