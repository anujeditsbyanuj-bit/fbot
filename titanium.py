...
def _clone_all_app_handlers(clone: Client):
    ...
    # Help/Support/Plans/Status (and their inline callbacks) get their own
    # explicit, dedicated registration right after this function runs (see
    # _get_clone_client below) instead of relying on this wholesale copy —
    # skip them here so they don't end up registered twice (which would
    # mean two "Help" replies, two Plans menus, etc. per tap on a clone).
    skip_callbacks |= {cb for cb, _ in (_wiring.get("menu_message_handlers") or [])}
    skip_callbacks |= {cb for cb, _ in (_wiring.get("menu_callback_handlers") or [])}
...
async def _get_clone_client(token: str, owner_id: int) -> Client:
    ...
    # Help/Support/Plans/Status menu buttons and their inline callbacks:
    # registered here explicitly (group 0, same as everywhere else) rather
    # than only relying on the wholesale copy above — this was the actual
    # ❓ Help / ☎️ Support-button-does-nothing-on-clones bug. These handler
    # functions/filters were being threaded all the way through from
    # main.py into menu_message_handlers/menu_callback_handlers for
    # exactly this purpose, but nothing ever iterated the lists to
    # actually call clone.add_handler() with them — so they sat unused
    # ("kept for backward compatibility") while every clone quietly
    # depended entirely on the generic copy loop above, which skips
    # negative groups and reuses the exact same MessageHandler/
    # CallbackQueryHandler *objects* the main app itself is dispatching
    # on. Explicit registration here removes that dependency for exactly
    # the handlers users interact with constantly on every clone.
    menu_message_handlers = _wiring.get("menu_message_handlers") or []
    for cb, filt in menu_message_handlers:
        clone.add_handler(MessageHandler(cb, filt & filters.private))
    menu_callback_handlers = _wiring.get("menu_callback_handlers") or []
    for cb, filt in menu_callback_handlers:
        clone.add_handler(CallbackQueryHandler(cb, filt))
